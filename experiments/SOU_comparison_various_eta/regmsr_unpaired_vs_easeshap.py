...
- gameSOUStructuredGaussian with alpha in {sqrt(0.25), sqrt(0.5), sqrt(0.75)}.
- n = 40 players, n^2 random high-order unanimity terms, and exact SOU
  semivalue ground truth.
...
Ground truth
------------
- Analytic SOU Shapley values from gameSOUStructuredGaussian.get_semivalue().
...
def run_path(alpha: float, run_idx: int) -> Path:
    return RUNS_DIR / f"{alpha_label(alpha)}_run{run_idx:02d}.pkl"
...
def groundtruth_worker(alpha: float, force: bool):
    ...
    start = time.time()
    game = gameSOUStructuredGaussian(**game_args(alpha))
...
def main() -> None:
    ...
    logger.info("game: n=%d, alphas=%s, num_high_order=%d, sigma2=%s", N, ALPHAS, NUM_HIGH_ORDER, SIGMA2)
