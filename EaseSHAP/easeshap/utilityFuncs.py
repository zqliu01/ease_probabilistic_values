...
_FLOAT128 = getattr(np, "float128", np.longdouble)
...
class gameSOU:
    ...
    def get_semivalue(self, *, semivalue, semivalue_param=None):
        if semivalue == "shapley":
            ...
            tmp = np.power(semivalue_param, self.sizes - 1, dtype=_FLOAT128)
...
def generate_sou_game(num_player, d, seed=42):
    """
    Generate the Faithful-GSV Section 4.1 SOU benchmark terms.

    Returns a benchmark-compatible `(sets, alpha)` pair: `sets` is a list of
    player-index sets and `alpha`
    contains the unnormalized term weights. Package utilities normalize the
    resulting game by the actual number of terms `d`.
    """
    ...
    sets = []
    alpha = np.zeros(d, dtype=np.float64)
    for idx in range(d):
        size = int(rng.randint(1, num_player))
        pos = rng.choice(pos_range, size=size, replace=False)
        sets.append(set(int(player) for player in pos))
        alpha[idx] = float(group_weight[pos % 4].mean())
    return sets, alpha


def sv_sou_true(num_player, sets, alpha):
    """Exact Shapley value for SOU terms normalized by the number of terms."""
    num_player = int(num_player)
    alpha = np.asarray(alpha, dtype=np.float64)
    if len(sets) != len(alpha):
        raise ValueError("`sets` and `alpha` must have the same length.")

    value = np.zeros(num_player, dtype=np.float64)
    d = float(len(alpha))
    if d == 0.0:
        return value

    for cur_set, weight in zip(sets, alpha):
        size = len(cur_set)
        if size <= 0:
            continue
        contribution = float(weight) / (d * float(size))
        for player in cur_set:
            value[int(player)] += contribution
    return value
...
class gameSOUStructuredGaussian:
    ...
    When ``max_high_order_size`` is specified below ``num_player - 1``, sampled
    high-order coalitions have cardinality in ``[3, max_high_order_size]``.
    The default ``None`` retains the legacy range ``[3, num_player - 1]``.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if max_high_order_size is None:
            effective_max_high_order_size = num_player - 1
        else:
            if isinstance(max_high_order_size, (bool, np.bool_)) or not isinstance(
                max_high_order_size, (int, np.integer)
            ):
                raise ValueError(
                    "`max_high_order_size` must be an integer at least 3 or None, "
                    f"got {max_high_order_size!r}."
                )
            max_high_order_size = int(max_high_order_size)
            if max_high_order_size < 3:
                raise ValueError(
                    "`max_high_order_size` must be at least 3 when specified, "
                    f"got {max_high_order_size!r}."
                )
            effective_max_high_order_size = min(max_high_order_size, num_player - 1)
        self.max_high_order_size = (
            None if effective_max_high_order_size == num_player - 1 else effective_max_high_order_size
        )
        self.effective_max_high_order_size = effective_max_high_order_size

        self.allow_duplicate_high_order = bool(allow_duplicate_high_order)
        if not self.allow_duplicate_high_order:
            num_possible_high_order = sum(
                math.comb(num_player, k)
                for k in range(3, effective_max_high_order_size + 1)
            )
            if num_high_order > num_possible_high_order:
                raise ValueError(
                    "`num_high_order` exceeds the number of unique coalitions "
                    f"with cardinality in [3, {effective_max_high_order_size}]."
                )
        ...
        os.makedirs(path, exist_ok=True)
        cache_stem = "n_player={};alpha={:.12g};n_high={};sigma2={:.12g};seed={};duplicate={}".format(
            num_player,
            alpha,
            num_high_order,
            sigma2,
        ...
        if self.max_high_order_size is not None:
            cache_stem += f";max_high={self.max_high_order_size}"
        data_saved = os.path.join(path, cache_stem + ".npz")
    ...
    @staticmethod
    def _sample_high_order_coalitions(
        rng,
        num_player,
        num_high_order,
        allow_duplicates,
        ...
        s_range = np.arange(3, max_high_order_size + 1)
        pos_range = np.arange(num_player)
...
class gameSOUStructuredGaussianBitset(gameSOUStructuredGaussian):
    """
    API-compatible faster evaluator for ``gameSOUStructuredGaussian``.

    The generated game data are identical to ``gameSOUStructuredGaussian``.
    Only ``evaluate`` changes: singleton/pair terms are evaluated directly,
    while high-order random unanimity terms use packed uint64 containment
    checks instead of scanning every player in every term.
    """

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self._build_fast_eval_cache()

    @staticmethod
    def _pack_bool_rows(X, n):
        ...
        packed = np.zeros((X_2d.shape[0], num_words), dtype=np.uint64)
        row_idx, col_idx = np.nonzero(X_2d)
        if len(row_idx):
            word_idx = col_idx // 64
            bit_idx = (col_idx % 64).astype(np.uint64, copy=False)
            bit_values = np.left_shift(np.uint64(1), bit_idx)
            np.bitwise_or.at(packed, (row_idx, word_idx), bit_values)
        return packed

    @classmethod
    def _pack_bool_mask(cls, mask, n):
        return cls._pack_bool_rows(mask, n)[0]

    def _build_fast_eval_cache(self):
        n = self.num_player
        low_mask = self.component == 0
        singleton_mask = low_mask & (self.sizes == 1)
        pair_mask = low_mask & (self.sizes == 2)
        high_mask = self.component == 1

        self._singleton_weights = np.zeros(n, dtype=np.float64)
        singleton_rows = np.flatnonzero(singleton_mask)
        if len(singleton_rows):
            singleton_players = np.argmax(self.coalitions[singleton_rows], axis=1)
            np.add.at(self._singleton_weights, singleton_players, self.weights[singleton_rows])

        self._pair_weights = np.zeros((n, n), dtype=np.float64)
        pair_rows = np.flatnonzero(pair_mask)
        if len(pair_rows):
            _row_pos, pair_cols = np.nonzero(self.coalitions[pair_rows])
            pair_players = pair_cols.reshape(len(pair_rows), 2)
            left = pair_players[:, 0]
            right = pair_players[:, 1]
            np.add.at(self._pair_weights, (left, right), self.weights[pair_rows])
            np.add.at(self._pair_weights, (right, left), self.weights[pair_rows])

        self._high_weights = self.weights[high_mask].astype(np.float64, copy=True)
        self._high_bits = self._pack_bool_rows(self.coalitions[high_mask], n)

    def evaluate(self, subset):
        subset = np.asarray(subset[:self.num_player], dtype=bool)
        active_players = np.flatnonzero(subset)

        value = float(self._singleton_weights[active_players].sum())
        if len(active_players) >= 2:
            value += 0.5 * float(self._pair_weights[np.ix_(active_players, active_players)].sum())

        if len(self._high_weights):
            subset_bits = self._pack_bool_mask(subset, self.num_player)
            active_high = np.all(
                (self._high_bits & subset_bits[None, :]) == self._high_bits,
                axis=1,
            )
            value += float(self._high_weights[active_high].sum())
        return value
